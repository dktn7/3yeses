...
export default function GalleryPage() {
  ...
  const handleFileUpload = (e: any) => {
    ...
    if (!type) {
      setNotification({ show: true, type: 'error', message: 'Invalid file type' });
      return;
    }
  ...
  const handleUrlUpload = async (type: string, url: string) => {
    ...
    if (!validationResult.isValid) {
      ...
      setNotification({ show: true, type: 'error', message: validationResult.error || 'Invalid URL' });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNotification({ show: true, type: 'success', message: 'Media added successfully', action: 'upload' });
        ...
        setNotification({ show: true, type: 'error', message: error.error || 'Upload failed' });
      }
    } catch (error) {
  ...
  const extractVideoThumbnail = (timestamp: number) => {
    if (!pendingFile || !pendingFile.type.startsWith('video/')) {
      setNotification({ show: true, type: 'error', message: 'No video selected' });
      return;
    }
    ...
    const extractFrame = () => {
      ...
      try {
        ...
        if (!ctx) {
          setNotification({ show: true, type: 'error', message: 'Canvas context failed' });
        ...
        if (thumbUrl && thumbUrl.length > 100) {
          ...
          setNotification({ show: true, type: 'success', message: 'Thumbnail extracted successfully', action: 'extract' });
        } else {
          throw new Error('Invalid thumbnail data');
        }
      } catch (err) {
        console.error('Thumbnail extraction error:', err);
        setNotification({ show: true, type: 'error', message: 'Could not extract thumbnail from video' });
      } finally {
        URL.revokeObjectURL(fileUrl);
      }
    };
    ...
    setTimeout(() => {
      if (!resolved && video.videoWidth > 0) {
        ...
        setNotification({ show: true, type: 'error', message: 'Video failed to load' });
        resolved = true;
        URL.revokeObjectURL(fileUrl);
      }
    }, 2000);
  ...
  const confirmUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setMedia([data.media, ...media]);
        setNotification({ show: true, type: 'success', message: 'Media uploaded successfully', action: 'upload' });
        setShowUploadModal(false);
        setPendingFile(null);
        setPendingTitle('');
        setPendingDescription('');
        ...
        const error = await response.json();
        setNotification({ show: true, type: 'error', message: error.error || 'Upload failed', action: 'upload' });
      }
    } catch (error) {
      console.error('Upload error:', error);
      setNotification({ show: true, type: 'error', message: 'Upload failed' });
  ...
  const deleteMedia = async (id: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setNotification({ show: true, type: 'success', message: 'Media deleted', action: 'delete' });
      } else {
        setNotification({ show: true, type: 'error', message: 'Delete failed' });
      }
    } catch (error) {
  ...
  const confirmEdit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEditingItem(null);
        setNotification({ show: true, type: 'success', message: 'Media updated', action: 'edit' });
      } else {
        setNotification({ show: true, type: 'error', message: 'Update failed' });
      }
    } catch (error) {
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-600 dark:text-gray-400">
            Curate your portfolio with photos, videos, and audio
          </p>
        </div>
          ...
          <div className="bg-light-surface dark:bg-dark-surface rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col border border-gray-200 dark:border-gray-700 animate-in zoom-in duration-200" role="dialog" aria-modal="true">
            ...
            <div className="flex-shrink-0 p-8 pb-4">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Share your creative work with the world</p>
                </div>
                <button 
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-4 scrollbar-thin scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent">
              ...
              {(uploadStep === 'select' || (uploadStep === 'details' && !pendingFile && !pendingMediaUrl)) && (
                <div className="space-y-4 mb-8">
                  <label className="block text-base font-semibold text-gray-900 dark:text-white mb-3">Choose Upload Method</label>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <div className="text-center">
                        <p className="font-semibold text-gray-900 dark:text-white">From Device</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400">Choose a file</p>
                      </div>
                    </button>
                    ...
                    >
                      <div className="text-3xl">🔗</div>
                      <div className="text-center">
                        <p className="font-semibold text-gray-900 dark:text-white">From URL</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400">Paste a link</p>
                      </div>
                    </button>
                  </div>
                </div>
              ...
              {uploadStep === 'url-input' && !pendingFile && (
                <div className="space-y-5">
                  <div className="p-5 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-200 dark:border-blue-800">
                    <p className="text-sm font-medium text-blue-900 dark:text-blue-300 mb-3">🔗 Enter Media URL</p>
                    <p className="text-xs text-blue-800 dark:text-blue-400 mb-4">Supported: Direct image/video/audio URLs or embeds from popular platforms</p>
                    
                    <div className="space-y-4">
                      {/* Media Type Selector */}
                      <div>
                        <label className="block text-sm font-semibold text-gray-900 dark:text-white mb-2">Media Type</label>
                        <div className="grid grid-cols-3 gap-2">
                          {(['image', 'video', 'audio'] as const).map((type) => (
                            ...
                            >
                              {type === 'image' ? '🖼️' : type === 'video' ? '🎬' : '🎵'} {type.charAt(0).toUpperCase() + type.slice(1)}
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      {/* URL Input */}
                      <div>
                        <label className="block text-sm font-semibold text-gray-900 dark:text-white mb-2">URL</label>
                        <input
                          type="url"
                        ...
                        />
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">Must be a complete URL starting with https://</p>
                      </div>
                      ...
                      <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                        ...
                        <p className="text-xs text-green-800 dark:text-green-400 mt-1">
                          URLs are validated against blocked domains and malicious sites before upload.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-gray-900 dark:to-gray-800 rounded-xl border border-blue-200 dark:border-gray-700">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 dark:text-white capitalize">{pendingType}</p>
                      ...
                      <div className="space-y-4">
                        <div className={`grid ${pendingType === 'video' ? 'grid-cols-3' : 'grid-cols-2'} gap-2`}>
                          {pendingType === 'video' && (
                            <button
                              ...
                              className={`px-4 py-3 rounded-xl font-semibold text-sm transition-all ${pendingThumbnailMethod === 'video' ? 'bg-gradient-to-r from-primary-blue to-blue-600 dark:from-accent-red dark:to-red-600 text-white shadow-lg scale-105' : 'bg-light-surface dark:bg-dark-surface text-light-surface dark:text-dark-surface border border-gray-300 dark:border-gray-600 hover:border-primary-blue dark:hover:border-accent-red'}`}
                            >
                              🎬 From Video
                            </button>
                          )}
                        ...
                        {pendingThumbnailMethod === 'video' && videoPreviewUrl && pendingType === 'video' && (
                          <div className="space-y-3 p-4 bg-light-surface dark:bg-dark-surface rounded-xl border border-gray-200 dark:border-gray-700">
                            <div className="flex items-center justify-between">
                              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Frame Position</label>
                            ...
                            {extractedThumbnail && (
                              <div className="relative aspect-video rounded-xl overflow-hidden border-2 border-green-500 shadow-xl">
                                <Image src={extractedThumbnail} alt="Extracted thumbnail" fill className="object-cover" />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Give your work an eye-catching title..."
                    />
                  ...
                  {/* Description */}
                  <div>
                    <label className="block text-base font-semibold text-gray-900 dark:text-white mb-2">Description</label>
                    <textarea
                      ...
                      placeholder="Tell the story behind your work, your inspiration, techniques used..."
                    />
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5 flex items-center justify-between">
                      <span>Help viewers appreciate your creation</span>
                  ...
                  {pendingType === 'image' && (
                    <div className="p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-xl border border-blue-200 dark:border-blue-800/50">
                      <p className="text-sm text-blue-700 dark:text-blue-300 font-medium">💡 Image Best Practices</p>
                      <ul className="text-xs text-blue-600 dark:text-blue-400 mt-2 space-y-1">
                        <li>• Formats: JPG, PNG, WebP, GIF</li>
                        <li>• Max size: 10MB</li>
                        <li>• Recommended: 1920x1080 or higher for best quality</li>
                      </ul>
                    </div>
                  )}
                  ...
                  {pendingType === 'video' && (
                    <div className="p-4 bg-gradient-to-r from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 rounded-xl border border-red-200 dark:border-red-800/50">
                      <p className="text-sm text-red-700 dark:text-red-300 font-medium">💡 Video Best Practices</p>
                  ...
                  {pendingType === 'audio' && (
                    <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl border border-purple-200 dark:border-purple-800/50">
                      <p className="text-sm text-purple-700 dark:text-purple-300 font-medium">💡 Audio Best Practices</p>
                      <ul className="text-xs text-purple-600 dark:text-purple-400 mt-2 space-y-1">
                        <li>• Formats: MP3, WAV, OGG, AAC</li>
                        <li>• Max size: 100MB</li>
                        <li>• Add cover art thumbnail for visual appeal</li>
                      </ul>
                    </div>
                  )}
                </div>
              ...
              {pendingMediaUrl && (
                <div className="space-y-5">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-gray-900 dark:to-gray-800 rounded-xl border border-blue-200 dark:border-gray-700">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 dark:text-white capitalize">From URL</p>
                  ...
                  {pendingType === 'image' && (
                    <div className="p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-xl border border-blue-200 dark:border-blue-800/50">
                      <p className="text-sm text-blue-700 dark:text-blue-300 font-medium">💡 Image Tips</p>
                      <ul className="text-xs text-blue-600 dark:text-blue-400 mt-2 space-y-1">
                        <li>• Must be a direct image URL</li>
                  ...
                  {pendingType === 'video' && (
                    <div className="p-4 bg-gradient-to-r from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 rounded-xl border border-red-200 dark:border-red-800/50">
                      <p className="text-sm text-red-700 dark:text-red-300 font-medium">💡 Video Tips</p>
                      <ul className="text-xs text-red-600 dark:text-red-400 mt-2 space-y-1">
                        <li>• Must be a direct video URL or video platform embed</li>
                  ...
                  {pendingType === 'audio' && (
                    <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl border border-purple-200 dark:border-purple-800/50">
                      <p className="text-sm text-purple-700 dark:text-purple-300 font-medium">💡 Audio Tips</p>
                      <ul className="text-xs text-purple-600 dark:text-purple-400 mt-2 space-y-1">
                        <li>• Must be a direct audio URL</li>
            ...
            <div className="flex-shrink-0 px-8 pb-8 pt-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
              <div className="flex items-center justify-between gap-4">
                ...
                >
                  {uploadStep === 'url-input' ? 'Back' : 'Cancel'}
                </button>
                <button 
                ...
                >
                  {uploading ? (
                    <>
                      ...
                      <span>{uploadStep === 'url-input' ? 'Adding...' : 'Uploading...'}</span>
                    </>
                  ) : (
                    <>
                      <span>{uploadStep === 'url-input' ? 'Add from URL' : 'Upload to Gallery'}</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-light-surface dark:bg-dark-surface rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700/50">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Audio</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white mt-1">
      ...
      <div className="bg-light-surface dark:bg-dark-surface rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700/50">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Your Media</h2>
        </div>
        ...
        {media.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">
              Start building your portfolio by uploading photos, videos, or audio files
            </p>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        {item.createdAt ? new Date(item.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'Recently added'}
                      </span>
                    </div>
      ...
      {editingItem && (
          ...
          <div className="bg-light-surface dark:bg-dark-surface rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] flex flex-col border border-gray-200 dark:border-gray-700 animate-in zoom-in-95 duration-200">
            <div className="flex-shrink-0 px-6 md:px-8 py-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 mt-1">Preview and update your {editingItem.type.toLowerCase()}</p>
                </div>
                <button 
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">Preview</label>
              <div className="bg-black rounded-2xl overflow-hidden shadow-2xl">
                {editingItem.type === 'image' ? (
                  <div className="relative aspect-video w-full">
                    <Image
                      src={editingItem.url}
                      alt={editingItem.title || 'Image preview'}
                      fill
            ...
            <div className="space-y-4">
              ...
              {(editingItem.type === 'video' || editingItem.type === 'audio') && (
                <div>
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">Thumbnail</label>
                  <div className="space-y-3">
                    ...
                    {thumbnailMethod === 'video' && editingItem.type === 'video' && (
                      <div className="space-y-3 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700">
                        <p className="text-sm text-gray-600 dark:text-gray-400">Extract thumbnail from your video</p>
                        <div className="flex items-center justify-between">
                          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Select Frame</label>
                        ...
                        {extractedThumbnail && (
                          <div className="relative aspect-video rounded-lg overflow-hidden border-2 border-green-500">
                            <Image src={extractedThumbnail} alt="Extracted thumbnail" fill className="object-cover" />
                            <div className="absolute top-2 right-2 bg-green-500 text-white text-xs px-2 py-1 rounded-full font-medium">✓ Ready</div>
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Optional: Leave empty to keep current thumbnail or fallback</p>
                </div>
              )}
            </div>
            </div>
          ...
          <div className={`rounded-2xl shadow-2xl border-2 px-6 py-4 min-w-[320px] backdrop-blur-md ${
            notification.type === 'success'
              ? notification.action === 'upload'
                ...
                ? 'bg-purple-50 dark:bg-purple-900/90 border-purple-500 dark:border-purple-400'
