...
import { 
  SIGNUP_CATEGORIES, 
  getSuggestedSkills, 
  validateSkillsForCategory, 
  findBestCategoryMatch,
  type CategoryMatch 
} from '@/lib/categoryMapping';

const TALENT_CATEGORIES = SIGNUP_CATEGORIES.map(cat => ({
  value: cat.value,
  label: cat.label,
  description: cat.description
}));

const COMMON_SKILLS = SIGNUP_CATEGORIES.reduce((acc, cat) => ({
  ...acc,
                ...
                {formData.category && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {COMMON_SKILLS[formData.category as keyof typeof COMMON_SKILLS]?.map(skill => (
