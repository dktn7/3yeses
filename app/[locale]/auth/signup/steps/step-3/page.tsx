...
export default function SignupStep3() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // TODO: Upload files to ImageKit before submitting
            ...
            <div>
              ...
              {profilePhotoPreview ? (
                <div className="relative inline-block">
                  <Image src={profilePhotoPreview} alt="Profile" width={128} height={128} className="rounded-full object-cover border-4 border-blue-600" />
                  <button
                    type="button"
                    onClick={() => {
                ...
                <div className="grid grid-cols-4 gap-3">
                  {portfolioPreview.map((preview, index) => (
                    <div key={index} className="relative group">
                      <Image src={preview} alt={`Portfolio ${index + 1}`} width={96} height={96} className="w-full h-24 object-cover rounded-lg" />
                      <button
                        type="button"
