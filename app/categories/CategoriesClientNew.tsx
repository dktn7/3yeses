...
import { CheckCircle, Users, Music, Zap, Camera, UserCheck, Filter, X, Search, MapPin, Star, Calendar, Heart, Settings, BookOpen, Monitor, Trophy, Clapperboard, Sparkles, SlidersHorizontal, ArrowLeft, Loader2 } from 'lucide-react';
  ...
  if (isCombinedMode) {
    const combinedMediaItems = combinedTalents.flatMap((talent: any) => 
      (talent.portfolio || []).map((item: any, index: number) => ({
        ...
        thumbnail: item.thumbnail || (item.type?.toLowerCase() === 'video' ? undefined : item.url),
        talentProfile: {
          id: talent.id,
                  ...
                  >
                    <div className="flex flex-col h-full">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-700 dark:text-gray-200 group-hover:text-white transition-colors mt-1">
                          {subcategory.description}
                        </p>
                      </div>
                      {subcategory._count?.talentProfiles > 0 && (
                        <span className="self-start text-xs px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 group-hover:bg-white/20 group-hover:text-white transition-colors mt-2">
                          {subcategory._count.talentProfiles} talents
                        </span>
                      )}
                    </div>
                  </div>
